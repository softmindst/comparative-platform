...
const obtener_configuracion_general = async function(req,res){
    if(req.user){
        let config = await Configuracion_general.findById({_id:'67bcfc8b236442dd2cddc971'});
...
const actualizar_configuracion_general_admin = async function(req,res){
    if(req.user){
        let id = '67bcfc8b236442dd2cddc971';
        let data = req.body;
...
const actualizar_configuracion_finanzas_admin = async function(req,res){
    if(req.user){
        let id = '67bcff2c236442dd2cddc981';
        let data = req.body;
...
const obtener_configuracion_finanza = async function(req,res){
    if(req.user){
        let config = await Configuracion_finanza.findById({_id:'67bcff2c236442dd2cddc981'});
        res.status(200).send({data:config})
    }else{
        res.status(403).send({data:undefined,message:'NoToken'});
    }
